...
class SiteBuildTests(unittest.TestCase):
    def test_build_contains_runtime_data_and_visuals(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertTrue((output / "charts" / "daily_brief.svg").exists())
            html = (output / "index.html").read_text(encoding="utf-8")
            script = (output / "app.js").read_text(encoding="utf-8")
            self.assertIn('id="share-button"', html)
            self.assertIn('id="cross-asset-grid"', html)
            self.assertIn('id="event-chain-list"', html)
            self.assertIn("navigator.share", script)
            self.assertIn("renderCrossAssets", script)
            self.assertIn("renderEventChains", script)
