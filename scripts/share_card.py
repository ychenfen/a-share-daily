...
def build_svg(analysis, global_data, latest):
    ...
    health_text = " · ".join(
        ...
        for key, label in (("markets", "MARKETS"), ("vix", "VIX"), ("us10y", "US10Y"), ("news", "NEWS"))
    )
    ...
    out = [
        ...
        f'<text x="727" y="184" fill="{score_color(float(shanghai.get("pct") or 0))}" font-size="28" font-weight="700">{xml(signed(shanghai.get("pct")))}</text>',
    ...
    for index, (label, pct, stale) in enumerate(market_cells):
        x = 58 + index * 271
        color = score_color(float(pct or 0))
        out.extend([
            f'<text x="{x}" y="452" fill="#9aa5ad" font-size="12" letter-spacing="1.5">{xml(label)}</text>',
            f'<text x="{x}" y="496" fill="{color}" font-size="30" font-weight="700">{xml(signed(pct))}</text>',
            f'<text x="{x + cell_width}" y="452" fill="#68737b" font-size="10" text-anchor="end">{"STALE" if stale else "LATEST"}</text>',
        ])
        if index < len(market_cells) - 1:
            out.append(f'<line x1="{x + cell_width + 7}" y1="432" x2="{x + cell_width + 7}" y2="506" stroke="#273037"/>')

    out.extend([
        '<rect x="58" y="530" width="1084" height="54" fill="#101519" stroke="#273037"/>',
        '<text x="78" y="563" fill="#e9c86f" font-size="12" font-weight="700" letter-spacing="1.5">RESEARCH NOTE</text>',
        f'<text x="233" y="563" fill="#f2ede4" font-size="17">{xml(advice)}</text>',
